...
export const createAccount = async (rawData: FormData) => {
    ...
    const account = await pgDocker.account.create({
        data,
    })
...
const CreateWorkSessionSchema = z.object({
    accountId: z.string(),
    startsOn: z.preprocess(nullifyEmptyString, z.coerce.date().optional().nullable()),
    description: z.string().min(1).max(120).optional().nullable(),
    hours: z.preprocess(nullifyEmptyString, z.coerce.number().min(0).max(24).optional().nullable()),
})
...
export const createWorkSession = async (rawData: FormData) => {
    ...
    const workSession = await pgDocker.workSession.create({
        data,
    })
...
export const updateWorkSession = async (rawData: FormData) => {
    ...
    const workSession = await pgDocker.workSession.update({
        data: {
...
export const deleteWorkSession = async (rawData: FormData) => {
    ...
    const session = await pgDocker.workSession.delete({ where: { id } })
